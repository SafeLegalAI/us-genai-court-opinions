...
- **2026-09-08** — Official-copy resolution: GovInfo (USCOURTS) API by docket with the quoted passage required to match; state-court archives by hand-briefed agents; CourtListener RECAP search within Free Law Project's published limits. 298 of 566 rows now read from the court's own copy or the public RECAP archive (from 53). 268 remain read from a public copy of the court's PDF: the court does not publish the order online, or its site refuses the identified bot (69 rows, queued for hand download), or the host timed out.
...
> SafeLegalAI (Cognesio LLP), "US court decisions on generative AI", v0.2.0, 2026-09-08. https://huggingface.co/datasets/safelegalaidata/us-genai-court-opinions — CC BY 4.0. Canonical: https://safelegalai.com/courts

```bibtex
@dataset{safelegalai_us_genai_court_opinions_0_2_0,
